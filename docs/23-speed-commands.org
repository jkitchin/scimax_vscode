...
5. *No jump is in progress* (see below)

** ⚠️ Speed Commands During a Jump

The avy-style jump commands (see [[file:21-navigation.org][Navigation]]) read
your keystrokes straight from the editor, so a single-key speed command would
otherwise swallow the character you meant as a jump label. Every single-key
binding in the extension is therefore suspended while a jump is running, using
the =!scimax.jumpActive= context. This covers the speed commands at headings,
source blocks, LaTeX sections and environments, BibTeX entries, and the agenda
buffer.

The same guard applies to the chorded bindings that start with Escape
(=Escape a=, =Escape b=, =Escape -=, =Escape Escape=). Without it VS Code would
treat Escape as the start of a chord and wait for a second key instead of
cancelling the jump, leaving the labels on screen.

Speed commands come back the moment the jump ends, whether you jumped, pressed
Escape, or let it time out.
...
| P   | Set property | Set a property; fuzzy-matches existing property names and values, and allows new ones (also bound to =C-c x p=) |
