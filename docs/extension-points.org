...
Each exporter is a *subdirectory* containing =manifest.json=, placed in one of
the searched locations:

| Location                             | Notes                                     |
|--------------------------------------+-------------------------------------------|
| ={scimax.directory}/exporters/=      | Default =~/scimax/exporters/=             |
| =~/.scimax/exporters/=               | Always searched                           |
| =$XDG_CONFIG_HOME/scimax/exporters/= | Only when =XDG_CONFIG_HOME= is set        |
| =<workspace>/.scimax/exporters/=     | Per-project exporters                     |
| =scimax.export.customExporterPaths=  | Any extra directories you list (=~= is expanded) |

A plain =exporters/= directory at the root of a project is *not* one of them --
it has to be =.scimax/exporters/=, or added to
...
Run =Scimax: Reload Custom Exporters= after adding or editing one; exporters are
loaded when the extension starts.

*** ⚠️ When an Exporter Does Not Appear

An exporter that fails to load is reported rather than silently skipped:
=Scimax: Show Custom Exporter Problems= lists each directory that could not be
read, with the reason, and opens its =manifest.json=. The export dispatcher also
shows a =[c !]= entry when something failed. The usual causes are:

- *Invalid JSON in manifest.json* -- most often a trailing comma:
  ="documentClass": "elsarticle",= followed by =}=. Comments and trailing
  commas are tolerated (with a warning), but anything else JSON rejects will
  keep the exporter from loading.
- *Missing =template= file* -- the =template= path is relative to the exporter
  directory.
- *A missing required field*: =id=, =name=, =parent=, =outputFormat=, =template=.
- *The directory is not in a searched location* (see the table above).
...
*** ⚠️ The Org Header an Exporter Needs

An exporter that declares =keywords= needs those keywords in the document
(=#+TO:=, =#+FROM:=, =#+SUBJECT:= for a memo). You do not have to remember
them:

- =Scimax: Insert Custom Exporter Header= inserts the header for an exporter you
  pick, with defaults filled in and required keywords left as =<<<PLACEHOLDER>>>=.
- The same header appears in the template pickers (=Scimax: Insert Template= /
  =Insert Org Template=) under the *Custom Exporters* category, named after the
  exporter.
- Exporting with a required keyword missing warns and offers to insert the
  header, instead of quietly writing =[NOT FOUND: to]= into the output.

Ship a real skeleton with an exporter by adding an =orgTemplate= to the
manifest -- a =.org= file in the exporter directory:
...
That file may use the [[file:35-templates.org][template]] conventions: ={{author}}=, ={{date}}=,
=${1:tab stops}=, =$0= for the final cursor position, and =<<<PLACEHOLDER>>>=
for text to replace by hand. Without =orgTemplate=, the header is generated from
the =keywords= in the manifest.

*** ⚠️ Running a Custom Exporter

| How                                          | Notes                                  |
|----------------------------------------------+----------------------------------------|
| =C-c C-e=, then =[c 1]=, =[c 2]=, ...        | Loaded exporters appear in the export dispatcher |
| =C-c C-e=, then =[c c]=                      | Opens the custom exporter picker       |
| =Scimax: Export with Custom Exporter=        | The same picker from the palette       |
| =#+LATEX_CLASS= routing                      | See below - the normal LaTeX/PDF exports use the exporter |

*** Automatic Routing by LaTeX Class

You do not have to remember to run the custom exporter by hand. Map a
=#+LATEX_CLASS= value to an exporter id and the normal LaTeX/PDF exports
(=C-c C-e l l=, =l p=, =l o=) use that exporter automatically:
...
With that setting, a document starting with
...
exports through the =cmu-memo= exporter on =C-c C-e l o=, no custom command
needed. The class name is matched case-insensitively, and the mapped value may
be any exporter id (the class and the exporter do not have to share a name).

*A class that names a loaded exporter routes to it even with no mapping.* There
is no =cmu-memo.cls= for LaTeX to load -- the exporter's template is what
supplies =\documentclass= -- so an unmapped =#+LATEX_CLASS: cmu-memo= would
otherwise fail the compile with =File `cmu-memo.cls' not found=. Map the class
to =none= if you really do have a class file of that name and want the built-in
backend.

Per-document overrides use the =#+EXPORTER= keyword, which needs no
configuration at all and is what the inserted header uses:

| Keyword                | Effect                                            |
|------------------------+---------------------------------------------------|
| =#+EXPORTER: cmu-memo= | Use this exporter, whatever =#+LATEX_CLASS= says  |
| =#+EXPORTER: none=     | Force the built-in LaTeX backend for this file    |

Notes:

- Routing applies to =l l= (writes the rendered =.tex=), =l p= (compiles the
  PDF), and =l o= (compiles and opens the PDF). Body-only export is never
  routed, since a template needs the full document.
- The CLI follows the same rules: =scimax export file.org --format pdf= routes
  through the mapped exporter. =--exporter <id>= still wins over routing.
- If the mapped exporter is not installed, you get a warning and the built-in
  LaTeX export runs instead. Run *Scimax: Reload Custom Exporters* after adding
  a new exporter directory.
...
| ={{{latexHeaders}}}=        | Document =#+LATEX_HEADER:= lines |

*** Document =#+LATEX_HEADER:= Lines

A document's =#+LATEX_HEADER:= and =#+LATEX_HEADER_EXTRA:= lines are collected
into the =latexHeaders= variable, so a letter can add packages the template
does not know about:
...
Place ={{{latexHeaders}}}= in the template wherever the headers belong. A
LaTeX template that does *not* mention =latexHeaders= gets them inserted
automatically just before =\begin{document}=, so older templates keep working.
