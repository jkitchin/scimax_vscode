...
| C-c `   | M-c     | Code (inline)                        |
| C-c =   | M-v     | Verbatim                             |
| C-c +   | M-x     | Strikethrough                        |
|         | M--     | Subscript                            |
|         | M-=     | Superscript                          |
| C-c '   |         | Command (Emacs-style, non-standard)  |

*Note:* Option/Alt key bindings (=M-b=, =M-i=, etc.) provide mnemonic shortcuts for the most common markup operations. On macOS, =M-= is the Option key.
