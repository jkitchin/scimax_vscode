...
| C-x b           | Fuzzy-pick recent org/md file  |
...
| C-c x p       | Set property (fuzzy-match name/value) |
| C-c C-q       | Set tags (pick from tags in use)      |
...
| S-<up>/M-<up>       | Move row up                       |
| S-<down>/M-<down>   | Move row down                     |
| S-<left>/M-<left>   | Move column left                  |
| S-<right>/M-<right> | Move column right                 |
| C-M-<up>            | Move cell up (swap)               |
| C-M-<down>          | Move cell down (swap)             |
| C-M-<left>          | Move cell left (swap)             |
| C-M-<right>         | Move cell right (swap)            |
...
| C-c j t | Jump to text (pause for labels) |
| C-c j w | Jump to word                    |
| C-c j l | Jump to line                    |
| C-c j o | Jump to symbol/outline          |
| C-c j s | Jump to subword                 |
| C-c j h | Jump to heading                 |
| C-c j ? | List the jump commands          |

While a jump is running, keystrokes go to the jump instead of the buffer:
Backspace deletes the last character typed and Escape cancels.

** Recenter

| Key | Action                                         |
|-----+------------------------------------------------|
| C-l | Recenter top/bottom (center → top → bottom)    |

Emacs-style =recenter-top-bottom=. Press =C-l= repeatedly while the cursor
stays put to cycle the cursor line through center, top, and bottom of the
viewport. Moving the cursor resets the cycle to center.
...
Single keys, active only inside an agenda buffer (see [[file:13-agenda.org][Agenda]]).
The buffer is read-only, so these keys are never ambiguous with typing.

| Key | Action                       |
|-----+------------------------------|
| RET | Jump to the item's source    |
| t   | Cycle the item's TODO state  |
| g   | Refresh                      |
| f   | Forward one period           |
| b   | Back one period              |
| .   | Return to the current period |
| q   | Close the buffer             |

Items are also links: Ctrl+click (Cmd+click on macOS) opens the source
heading. Right-click any line for a context menu with the same actions.
...
* Rectangle Commands (Emacs-style)

Emacs-style rectangle editing commands. A rectangle is defined by the column range and line range of the current selection.

| Key         | Action                                |
|-------------+---------------------------------------|
| C-x r k     | Kill rectangle (save to rect clip)    |
| C-x r M-w   | Copy rectangle (no delete)            |
| C-x r d     | Delete rectangle (no save)            |
| C-x r y     | Yank rectangle at point               |
| C-x r o     | Open rectangle (insert blank space)   |
| C-x r c     | Clear rectangle (replace with spaces) |
| C-x r S-n   | Number lines in rectangle             |
| C-x r t     | String rectangle (replace with text)  |

*Note:* The rectangle clipboard is separate from the system clipboard, matching Emacs behavior. Kill/copy saves to the rectangle clipboard; yank inserts from it.
