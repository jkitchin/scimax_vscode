...
The agenda buffer is a persistent, refreshable agenda in a full editor tab —
the closest analogue to Emacs =org-agenda-mode=. Unlike the quick pick, it
stays open; unlike the tree view, it gets the whole editor.

Open it with =C-c a v= ([[cmd:scimax.agenda.openBuffer]]), or from the agenda menu
(=C-c a a=), then choose a span (day, week, fortnight, or month):
...
The buffer is read-only, so it can never be edited by accident and never
prompts to save.
...
| Key   | Command                              | Action                       |
|-------+--------------------------------------+------------------------------|
| =RET= | [[cmd:scimax.agenda.buffer.goto]]        | Jump to the item's source    |
| =t=   | [[cmd:scimax.agenda.buffer.cycleTodo]]   | Cycle the item's TODO state  |
| =g=   | [[cmd:scimax.agenda.buffer.refresh]]     | Re-query the database        |
| =f=   | [[cmd:scimax.agenda.buffer.next]]        | Forward one period           |
| =b=   | [[cmd:scimax.agenda.buffer.previous]]    | Back one period              |
| =.=   | [[cmd:scimax.agenda.buffer.today]]       | Return to the current period |
| =q=   | -                                    | Close the buffer             |

Paging with =f= and =b= updates the buffer *in place* rather than opening a
new tab, so you can walk forward through weeks without accumulating editors.

*** ⚠️ Mouse Interaction

Every item line is a link. Ctrl+click (Cmd+click on macOS) opens the source
heading — VS Code reserves plain click for cursor placement, so a modifier is
required. Right-click gives a context menu with the same actions as the keys
above.

Cycling a TODO state briefly switches to the source file to make the edit, then
returns to the agenda and refreshes it. This is deliberate: it reuses the real
[[cmd:scimax.org.cycleTodo]] command, so repeating tasks advance their timestamps,
CLOSED timestamps are written, and org-depend blocking still applies — none of
which would hold if the agenda edited the file directly.

The buffer also refreshes itself whenever the database is reindexed, so it
never shows a stale snapshot. Press =g= to force a refresh.
...
A file's own =#+TODO:= line decides which of its keywords are done. Keywords
after the =|= are done, and those headings are left out of the agenda and TODO
list:
...
In files without a =#+TODO:= line, =DONE=, =CANCELLED= and =CANCELED= are done.
=scimax.agenda.doneStates= adds keywords that count as done in every file.

Done-ness is recorded when a file is indexed, so after editing a file's
=#+TODO:= line the agenda updates once that file is reindexed (on save, or via
=scimax db sync=).

| Setting                  | Default                     |
|--------------------------+-----------------------------|
| scimax.agenda.todoStates | [TODO, NEXT, WAITING]       |
| scimax.agenda.doneStates | [DONE, CANCELLED, CANCELED] |
...
1. *Check indexed files*:
   ...
   Add your org directories; they are stored in scimax.db.include
...
4. *Sync database* (for database agenda):
   #+BEGIN_SRC text
...
| scimax.db.exclude          | []      | Don't index these (adds to the built-in baseline) |
