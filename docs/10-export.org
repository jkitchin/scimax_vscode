...
*** ⚠️ Clipboard Exports (ox-clip style)
...
DOCX export generates Microsoft Word (.docx) documents with full formatting support, syntax-highlighted code blocks, embedded images, and bibliography generation.

*** Features

- Native Word document format (no external tools required)
- Syntax highlighting for code blocks using Shiki
- Embedded images from file links
- Built-in Word styles for consistent formatting
- Citation and bibliography support
- Tables with header row styling
- Nested lists (ordered, unordered, description)
- Special blocks (quote, verse, note, warning, tip)
- Footnotes using Word's native footnote system
...
Source blocks are exported with syntax highlighting using the Shiki library:
...
The code block appears in Word with proper colors matching popular editor themes (github-light by default).

*Supported languages include:* Python, JavaScript, TypeScript, Java, C, C++, Rust, Go, Ruby, Shell, SQL, R, Julia, and many more.
...
DOCX export integrates with the citation processor for full bibliography support:
...
The bibliography section is automatically generated at the end of the document.
...
- Table of contents requires manual update in Word (Insert → Update Field)
- Complex nested tables may need adjustment
- Some advanced LaTeX math may not render (use simple equations)
- Internal links work within the document but external file links are text-only

** ⚠️ Clipboard Export (ox-clip)
...
| \vert{}  | t/nil              | t       | Include tables                         |
| ^        | t/nil/{}           | t       | Sub/superscripts ({}=braces required)  |
| :        | t/nil              | t       | Include fixed-width sections           |
| \n       | t/nil              | nil     | Preserve line breaks                   |
