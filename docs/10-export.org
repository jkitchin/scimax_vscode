...
These three can be routed to a custom exporter based on the document's
=#+LATEX_CLASS=, so a memo or report class exports correctly without running a
separate command. See [[*⚠️ Custom Exporters by LaTeX Class][Custom Exporters by LaTeX Class]].

*** Beamer Exports
- `[s b]' Beamer (LaTeX) - Export to Beamer .tex file
- `[s p]' Beamer PDF - Compile Beamer to PDF
- `[s o]' Beamer PDF and open - Compile and open with system viewer

See [[file:41-export-beamer.org][Beamer Export]] for the keyword/property reference.
...
Every loaded [[file:extension-points.org::*Custom Exporters][custom exporter]] is listed by name on a digit key, so a
user-defined backend is reachable the same way the built-in formats are:

- `[1]', `[2]', ... - run that exporter
- `[c]' Custom exports - the exporter picker
- `[!]' - shown only when an exporter failed to load; explains why

If an exporter you wrote is missing here, run =Scimax: Show Custom Exporter
Problems=: an unparseable =manifest.json= (a trailing comma is the classic one)
is the usual reason.
...
*Quick keybinding:* =C-c k= copies current subtree as rich HTML.
...
Document classes that need a template (letters, memos, theses) are handled by
[[file:extension-points.org][custom exporters]]. Map the class to an exporter id once, and the standard
LaTeX/PDF exports use it automatically:
...
does not have to match the class name. A class that *is* the name of a loaded
exporter routes to it even without a mapping -- there is no =cmu-memo.cls= for
LaTeX to find, the exporter's template supplies =\documentclass=.

Per-document control (=#+EXPORTER= needs no configuration, and is what
=Scimax: Insert Custom Exporter Header= writes):
...
=#+LATEX_HEADER:= lines work with custom exporters too - they land in the
template's =latexHeaders= variable, or, for templates that do not use that
variable, are inserted just before =\begin{document}=:
...
- *Reword* to drop the sign: =50,000 USD= or =USD 50,000=.
- *Inline verbatim*: ==$50,000== exports as =\texttt{\$50,000}= (monospace, but
  correct).
- *Raw LaTeX snippet* for the sign only: ~@@latex:\$@@50,000~ emits a literal
  =\$= just in the LaTeX backend.
...
The bibliography step in each is guarded, so a document with no citations
...
same command. See
[[file:33-latex-navigation.org::*Build Profiles for TeX Files][Build Profiles for TeX Files]].
...
1. =rsvg-convert= (from librsvg -- fastest, recommended)
2. =cairosvg= (a Python package)
3. =inkscape= (version 1.x)
...
- The *first LaTeX error* (message plus the =.tex= line it points at) is printed,
  e.g. =LaTeX error: Unknown graphics extension: .svg. (report.tex:95)=, along
  with the path to the full =.log=. Add =--show-log= (aliases =--verbose=, =-v=) to
  also print the tail of the log.
- The *incomplete PDF is removed* on failure. A fatal LaTeX error can still leave
  behind a =report.pdf= that looks complete but is broken (a classic case: LaTeX
  aborts before BibTeX runs, so every citation is unresolved). Removing it means
  a failed build can't be mistaken for a good export. Any pre-existing
  =--output= target is left untouched, since a failed run never writes to it.
- With =--json=, the failure object includes =latex_error=, =tex_line=, =log_file=,
  and =removed_partial_pdf=.
...
- *Do not hand-edit =report.tex= expecting the change to survive.* Fix the problem
  in the =.org= source and re-export. With in-pipeline SVG→PDF conversion (see
  /SVG Images/ above) and clear failure reporting (see /Error Handling/ above),
  the two common reasons to reach for a =.tex= patch are already handled inside
  the export.
- Some LaTeX setups also define a =latexmk= custom dependency that rebuilds
  =.tex= from =.org=; where that exists, running =latexmk= on the =.tex= regenerates
  it too, so a =.tex=-level patch is doubly futile.
- If you truly need a one-off from an edited =.tex= (e.g. to test a fix for an
  export bug), compile it *directly* with the LaTeX engine and do **not** re-run
  =scimax export= (which would regenerate it):
  #+BEGIN_SRC bash
...
This exports your org file to PDF and opens it in a VS Code panel with full
bidirectional synchronization support.

**** Forward Sync (Org → PDF)

Double-click a word in your org source to jump to that location in the PDF.
The word is highlighted with a yellow flash for easy identification.
...
Double-click a word in the PDF viewer to jump back to the corresponding
line in your org source file.
...
The system maintains a mapping between org source lines and generated LaTeX
lines. When you export to PDF, this mapping is stored so that both forward
and inverse sync can translate between org positions and PDF positions.

Note: The sync is approximate due to the org → LaTeX transformation.
It works best with paragraph-level precision.

** ✅ Markdown Export
CLOSED: [2026-01-17 Sat 09:18]
...
** ✅ DOCX Export
CLOSED: [2026-01-24 Sat 17:40]

DOCX export generates Microsoft Word (.docx) documents using [[https://pandoc.org/][Pandoc]] for high-quality conversion with proper equation rendering and bibliography support.
...
Clipboard export (inspired by [[https://github.com/jkitchin/ox-clip][ox-clip]]) copies formatted org content to the clipboard, enabling pasting into email clients, Word, Google Docs, and other applications that accept rich text.
...
|-----+------------------+----------------------------------------------|
| h   | HTML (rich)      | Copy as formatted text for email/Word        |
| H   | HTML (source)    | Copy raw HTML source code                    |
| l   | LaTeX            | Copy LaTeX source code                       |
| m   | Markdown         | Copy Markdown source code                    |
...
- [[cmd:scimax.org.exportHtmlOpen]]
- [[cmd:scimax.org.exportLatexOpen]]
- [[cmd:scimax.org.exportMarkdown]]
...
| email        | t, nil             | nil     | Include email in output                                       |
...
| p            | t, nil             | nil     | Include statistics cookies ([1/3], [50%])                     |
...
| ^            | t, nil, {}         | {}      | Sub/superscripts (default: only a_{b}/a^{b}; t: also a_b/a^b) |
...
| email    | t/nil              | nil     | Include email (from #+EMAIL)           |
| tags     | t/nil/not-in-toc   | t       | Include tags in headlines              |
...
- [[file:25-commands.org][Commands]] - Build profile and custom exporter commands
- [[file:36-manuscript.org][Manuscript]] - Multi-file LaTeX manuscripts
