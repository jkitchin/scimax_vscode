...
Move just the cell at the cursor, swapping it with the cell it displaces. The
cursor follows the cell, so you can repeat the key to walk a value across the
table.
...
- Vertical moves step *over* separator lines, so a cell can move into or out of
  a header row.
...
- Rows with fewer cells than the rest of the table are padded with empty cells
  so the swap lands in the right column.
