...
| scimax.externalTerminal.app      | string | ""      | macOS terminal app to open (auto-detect if empty) |
| scimax.externalTerminal.command  | string | ""      | Terminal executable, overrides auto-detection     |
| scimax.externalTerminal.args     | array  | []      | Arguments for that executable (=${dir}= expanded) |
...
- *OpenAlex*: Setting your email gives you access to the [[https://docs.openalex.org/how-to-use-the-api/rate-limits-and-authentication#the-polite-pool][polite pool]] with faster rate limits
...
*** ⚠️ scimax.externalTerminal.app / .command / .args

These control =Scimax: Open External Terminal Here=, which opens your real
terminal application in the directory of the current file. It is available on
the editor tab context menu and the explorer context menu, next to =Open
Terminal Here= (which uses VS Code's integrated terminal instead).

- *scimax.externalTerminal.app* (string, default `""'): macOS only. The
  application name passed to =open -a=, e.g. ="iTerm"=, ="Terminal"=,
  ="Ghostty"=, ="WezTerm"=, ="kitty"=, ="Alacritty"=, ="Warp"=. When empty,
  Scimax auto-detects an installed terminal, preferring iTerm.
- *scimax.externalTerminal.command* (string, default `""'): An executable to
  launch instead. Overrides auto-detection and =app= on every platform.
- *scimax.externalTerminal.args* (array of strings, default =[]=): Arguments
  for that executable. =${dir}= is replaced with the target directory, which is
  also used as the process working directory.

*Example* - always use Terminal.app on macOS:
#+begin_src json
{
  "scimax.externalTerminal.app": "Terminal"
}
#+end_src

*Example* - an explicit command with arguments:
#+begin_src json
{
  "scimax.externalTerminal.command": "alacritty",
  "scimax.externalTerminal.args": ["--working-directory", "${dir}"]
}
#+end_src

On Linux, Scimax tries =x-terminal-emulator=, =gnome-terminal=, =konsole=,
=xfce4-terminal=, =ghostty=, =wezterm=, =kitty=, =alacritty=, then =xterm=. On
Windows it uses Windows Terminal (=wt.exe=) when available and falls back to
=cmd.exe=.
...
graph. See [[file:03.5-task-dependencies.org][Task Dependencies]] for the full guide.

** ✅ Entity Selector (Contacts, Locations, …)
CLOSED: [2026-07-02 Thu 08:05]
...
mailto/maps/open-link actions). See [[file:03.7-contacts-and-locations.org][Contacts, Locations & Entity Selector]].
...
| scimax.db.exclude           | array   | =[]=                      | Extra patterns/paths to exclude        |
...
- *Type*: array of strings
- *Default*: =[]=
- *Description*: /Additional/ patterns or paths to exclude from indexing (globs
  and absolute paths). They are added to a built-in baseline that always
  applies — =node_modules=, VCS directories, build output, and editor state such
  as VS Code's local history. See
  [[file:18-database-search.org::*Ignore Patterns][Ignore Patterns]] for the
  full list and for why the baseline cannot be overridden.
...
CLOSED: [2026-05-03 Sun 07:59]

- *Type*: boolean
- *Default*: `true'
- *Description*: On activation, check whether any indexed files have changed on
  disk and show a one-time prompt offering to run *Refresh*. Useful for
  catching changes synced from another machine (e.g. Dropbox) while VS Code
  was closed. Skipped if `scimax.db.autoCheckStale' is on (which auto-refreshes
...
| scimax.ref.citationSyntax    | string | "org-ref-v3" | Citation syntax to insert |
| scimax.ref.citationLinkStyle | string | "plain" | Plain or bracketed cite links |
...
*** ⚠️ scimax.ref.citationSyntax

- *Type*: string (enum)
- *Default*: `"org-ref-v3"'
- *Options*: `"org-ref-v3"', `"org-ref-v2"', `"org-cite"'
- *Description*: Which citation syntax new citations are written in
  - `org-ref-v3' (recommended): =cite:&key1;&key2=
  - `org-ref-v2' (legacy): =cite:key1,key2=
  - `org-cite' (org 9.5+): =[cite:@key1;@key2]=

All three are *read* correctly regardless of this setting; it only controls
insertion.

*Example*:
#+begin_src json
{
  "scimax.ref.citationSyntax": "org-ref-v3"
}
#+end_src

*** ⚠️ scimax.ref.citationLinkStyle

- *Type*: string (enum)
- *Default*: `"plain"'
- *Options*: `"plain"', `"bracketed"'
- *Description*: Whether org-ref citations are inserted as a plain link or as a
  bracketed org link
  - `plain': =cite:&key=
  - `bracketed': =[[cite:&key]]= -- the form Emacs org-ref/scimax inserts

Set this to `bracketed' if you edit the same files in Emacs and want inserted
citations to look identical in both editors. Both forms are read, exported and
followed correctly either way. The setting is ignored when
=scimax.ref.citationSyntax= is `org-cite', which has its own brackets.

Note that a citation carrying pre/post notes (which contain spaces) is *always*
inserted in the bracketed form: a plain org link ends at the first space, so
=cite:&key p. 5= would leave "p. 5" outside the link.

*Example*:
#+begin_src json
{
  "scimax.ref.citationLinkStyle": "bracketed"
}
#+end_src
...
- *Description*: Backend used when translating org-cite citations
  (=[cite/STYLE/VARIANTS:@key]=) into LaTeX.
  - =bibtex= (default): emits =natbib= commands -- =\citep=, =\citet=,
    =\citealp=, =\Citet=, etc. Use with the standard bibtex/natbib stack.
  - =biblatex=: emits biblatex commands -- =\autocite=, =\textcite=,
    =\Autocite=, =\fullcite=, etc. Requires =\usepackage{biblatex}= and
    biber.

A per-document override is also supported via =#+cite_export: bibtex= or
=#+cite_export: biblatex=. See [[file:10-export.org][Export]] for the full
...
CLOSED: [2026-07-02 Thu 08:04]

See [[file:41-export-beamer.org][Beamer Export]] for the full feature reference.
...
- *Default*: ={}=
- *Description*: Makes the ordinary LaTeX/PDF exports (=C-c C-e l l=, =l p=,
  =l o=) use a custom exporter when the document declares a matching
  =#+LATEX_CLASS=. Keys are class names (matched case-insensitively), values
  are custom exporter ids.
...
See [[file:extension-points.org][Extension Points]] for how to define custom exporters.
...
- *Type*: string
- *Default*: =""=
- *Description*: Name of the [[file:10-export.org::*Build Profiles][build profile]] to use when a document does not
  choose one with =#+LATEX_BUILD:= and no nearby =latex-profiles.json= declares a
  =default=. Set to =none= to always use =scimax.export.pdf.compiler=.
...
- *Description*: Build profiles defined in settings, in the same shape as a
  =latex-profiles.json= file's =profiles= object. These override the built-in
  profiles of the same name and are available in every workspace, trusted or
  not. Project-local =latex-profiles.json= files override these in turn.
...
=bibtex= and =biber= expect. See [[file:10-export.org::*Build Profiles][Build Profiles]] for the full token and step
reference.
...
- *Type*: boolean
- *Default*: =true=
- *Description*: Show a tinted whole-line background behind each org heading,
  fading at deeper levels.
...
Configure the avy-style jump commands (=scimax.jump.*=). See
[[file:21-navigation.org][Navigation]] for how the jump flow works.

** Settings Overview

| Setting                       | Type    | Default                    | Description                                              |
|-------------------------------+---------+----------------------------+----------------------------------------------------------|
| scimax.jump.timeoutMs         | integer | 300                        | Pause with no typing before labels are drawn             |
| scimax.jump.labelChars        | string  | asdfjklghqweruiopzxcvbnmty | Label characters, in order of preference                 |
| scimax.jump.allVisibleEditors | boolean | true                       | Label targets in every visible editor, not just the active one |
| scimax.jump.dimBackground     | boolean | false                      | Grey out the text while labels are showing               |
| scimax.jump.labelBackground   | string  | #d33682                    | Label background color (any CSS color)                   |
| scimax.jump.labelForeground   | string  | #ffffff                    | Label text color (any CSS color)                         |

** Detailed Configuration

*** scimax.jump.timeoutMs

The equivalent of Emacs =avy-timeout-seconds=. After you stop typing your query
for this many milliseconds, the labels are drawn over the matches and further
keystrokes select a label rather than extending the query. Raise it if labels
appear while you are still typing; set it to 0 to label as soon as the query
matches anything.

*** scimax.jump.labelChars

The equivalent of Emacs =avy-keys=. Labels are assigned nearest-first from the
cursor, so the characters at the front of this string land on the closest
targets. Duplicate characters and whitespace are ignored, and at least two
distinct characters are required.

*** scimax.jump.allVisibleEditors

When true, jump commands label matches in every visible editor and selecting a
label in another split focuses that editor first. Set it to false to restrict
jumping to the active editor.

*** scimax.jump.dimBackground

The equivalent of Emacs =avy-background=. Recolors the visible text to the
muted line-number color while labels are showing, so the labels stand out. Off
by default, matching avy.

*** scimax.jump.labelBackground / scimax.jump.labelForeground

Labels are painted directly over the characters they mark, so the background
color must be opaque. Any CSS color works.
...
- [[file:21-navigation.org][Navigation]] - How jump commands work
- [[file:24-keybindings.org][Keybindings]] - Jump keybindings (=C-c j c=, =C-c j w=, …)
...
| scimax.agenda.doneStates  | array   | ["DONE", "CANCELLED", "CANCELED"] | Completed states             |
...
On activation, Scimax shows a one-shot toast if any deprecated agenda
indexing setting is set non-default in your config.
...
- *Description*: Patterns or paths to *hide* from the agenda view.
  Files matching these stay in the database (search still finds them);
  they just don't show up in day/week/month views. Supports both glob
  patterns (containing =*=) and absolute paths.

  To *not index* a file at all, use =scimax.db.exclude= instead.
...
- *Type*: array of strings
- *Default*: `["DONE", "CANCELLED", "CANCELED"]'
- *Description*: Completed TODO states. These add to the done keywords each
  file declares after =|= in its own =#+TODO:= line, which always count as done.
...
- *Description*: Granularity for =file+datetree= capture targets — controls the
  heading tree that captures are filed under. Missing headings are created
  automatically.
  ...
  Week mode uses the ISO week-numbering year, so a date in early January that
  belongs to the previous ISO year is filed under that year.
