...
*** ⚠️ scimax.highlighting.headingBackgroundBars
...
| scimax.capture.datetreeFormat    | string  | "day"   | Deprecated: has no effect                    |
| scimax.capture.autoSave          | boolean | true    | Deprecated: has no effect                    |
...
*** ⚠️ scimax.capture.defaultDirectory
...
- *Deprecated*: this setting has no effect. Datetree captures always use year > month > day granularity.

*** 👀 scimax.capture.autoSave

- *Deprecated*: this setting has no effect. Captures are always written directly to disk.
