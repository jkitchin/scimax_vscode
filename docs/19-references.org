...
** ✅ Multiple Citations
CLOSED: [2026-01-18 Sun 10:15]

Cite multiple sources by separating keys with commas or semicolons:

cite:kitchin-2015-examp-effec,kitchin-2025-beyon-fourt          # Version 2

cite:&kitchin-2015-examp-effec;&kitchin-2025-beyon-fourt        # Version 3


** 👀 Trailing punctuation after a cite: link
CLOSED: [2026-07-14 Tue]

Sentence punctuation immediately after an old-style =cite:= link is *not* absorbed
into the key. A colon is the common trap: writing

: ...as shown in cite:girgis-1991-reactivities:

exports =\cite{girgis-1991-reactivities}:= (the colon is left as text), not
=\cite{girgis-1991-reactivities:}= (which BibTeX cannot resolve, so the citation
would render as =?= and be silently dropped from the bibliography). Internal
colons in a key (e.g. =cite:smith:2020=) are preserved -- only a *trailing* colon
is treated as punctuation. Run =scimax cite check <file.org>= to catch any key
that is cited but not present in the =.bib=.

** ⚠️ Citation Links vs. Plain Links (Emacs round-trip)

An org-ref citation can be written two ways, and scimax VS Code reads, exports
and follows both:

| Form      | Example           | Notes                                     |
|-----------+-------------------+-------------------------------------------|
| plain     | =cite:&key=       | Ends at the first space                   |
| bracketed | =[[cite:&key]]=   | A real org link; what Emacs org-ref inserts |

Emacs org-ref/scimax inserts the bracketed form. If you edit the same files in
both editors and want inserted citations to match, set:
...
The bracketed form is also the only one that can carry pre/post notes, because a
*plain* org link ends at the first whitespace -- in =cite:&key p. 5= the "p. 5"
is ordinary text, while =[[cite:&key p. 5]]= is a citation with a postnote.
Scimax inserts the bracketed form automatically whenever you add notes,
regardless of this setting.

A description works too, and is kept when the citation is edited:

: [[cite:&kitchin-2015-examp-effec][see the examples paper]]
...
citet:&kitchin-2015-examp-effec;&kitchin-2025-beyon-fourt  
...
The citation's existing form is preserved -- syntax (v2/v3/org-cite), any
pre/post notes, and whether it is a plain or bracketed link, so
=[[cite:&key1]]= becomes =[[cite:&key1;&key2]]=.
...
2. *Better BibTeX plugin* - Install from https://retorque.re/zotero-better-bibtex/
...
| Emacs doesn't see the citation | Set =scimax.ref.citationLinkStyle= to =bracketed=     |
| VS Code loses focus after insert | This is an OS limitation; manually switch back to VS Code |

*Note on window focus:* After selecting citations in Zotero, VS Code may not automatically regain window focus. This is a macOS security limitation - VS Code extensions cannot programmatically bring windows to the foreground. Use Cmd-Tab or Shift-Tab to switch back to VS Code after the citation is inserted.


* ✅ Following Citation Links
CLOSED: [2026-01-17 Sat 10:18]

** ✅ Citation Link Actions
CLOSED: [2026-01-17 Sat 10:19]
...
| :CUSTOM_ID:     | =:CUSTOM_ID: label=       | Heading IDs           |
