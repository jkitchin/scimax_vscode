...
** ⚠️ How Image Overlays Work

Due to VS Code's limitations compared to Emacs overlays, the extension uses a combination of:

1. *Gutter Icons* - Small thumbnails displayed in the editor gutter (line margin)
2. *After-Content Decorations* - Emoji and dimension text displayed inline after the link
3. *Hover Previews* - Full-size image preview when hovering over image links
...
** ⚠️ Links with Descriptions

Add descriptive text that appears as a clickable link:

#+BEGIN_SRC org
[[./path/to/image.png][My beautiful diagram]]
[[file:./screenshots/demo.png][Demo Screenshot]]
#+END_SRC
...
See [[file:export.org][Export Documentation]] for more details on export attributes.
...
* ⚠️ Enabling and Disabling Image Display
...
- *Command ID:* scimax.imageOverlays.toggle

Use the command palette (Ctrl+Shift+P / Cmd+Shift+P) and search for "Toggle Image Overlays".
...
* ⚠️ Images from Source Block Results

** ⚠️ Automatic Image Display

When source blocks produce image output, the results are automatically displayed as overlays:
...
The resulting [[file:plot.png]] link automatically shows as an overlay.

** ⚠️ Jupyter Kernel Output
...
See [[file:jupyter.org][Jupyter Documentation]] for more details on Jupyter integration.

** ⚠️ R and Other Languages

R, Julia, and other languages that produce image files work similarly:

#+BEGIN_SRC org
,#+BEGIN_SRC R :results file graphics :file myplot.png
plot(1:10, (1:10)^2)
,#+END_SRC
...
** ⚠️ Scimax: Refresh Image Overlays

*Command ID:* scimax.imageOverlays.refresh
...
** ⚠️ Scimax: Clear Image Overlay Cache

*Command ID:* scimax.imageOverlays.clearCache
...
** ⚠️ Scimax: Show Image Overlay Cache Statistics

*Command ID:* scimax.imageOverlays.showStats
...
** ⚠️ Overlays Disappear When Editing
...
** ⚠️ Thumbnails Are Blurry or Corrupted
...
** ⚠️ Remote Images Don't Show

Remote HTTP/HTTPS images are *not displayed* as overlays by design. Only local files are supported for security and performance reasons.

To display remote images:
1. Download them locally
2. Reference the local copy in your org file

** ⚠️ Relative Paths Not Resolving

If relative paths aren't working:

1. *Use ./ prefix for clarity:*
   #+BEGIN_SRC org
   [[./images/photo.png]]  # Better
   [[images/photo.png]]    # Also works, but less clear
   #+END_SRC
...
** ⚠️ Performance-Focused Configuration
...
- [[file:links.org][Links]] - General link syntax and functionality
- [[file:source-blocks.org][Source Blocks]] - Executing code that generates images
- [[file:jupyter.org][Jupyter Integration]] - Working with Jupyter kernels
- [[file:export.org][Export]] - Exporting documents with images
- [[file:configuration.org][Configuration]] - Full configuration reference
...
- Previous: [[file:latex-preview.org][LaTeX Preview]]
- Next: [[file:agenda.org][Agenda]]
