...
* ⚠️ LaTeX Navigation and Structure
...
The outline view (VS Code's built-in outline panel) shows the structure of your LaTeX document:
...
| scimax.latex.jumpToSection            | C-c j         | Quick pick to jump to any section  |

** ⚠️ Next/Previous Section
...
The =jumpToSection= command (C-c j) opens a quick pick showing all sections with:
...
*Forward search* (C-c C-j): Jump from cursor position in source to corresponding PDF location.
...
** ⚠️ Supported Reference Commands
