...
3. *Files indexed with embeddings* - Run `scimax.db.refresh' after configuring
...
1. Run `Scimax: Refresh Database (incremental)' (`scimax.db.refresh').
   For a full embeddings rebuild, use `Scimax: Rebuild Database (from scratch)' (`scimax.db.rebuild').
...
*** 👀 Incremental Refresh (daily use)
Command: `Scimax: Refresh Database (incremental)' (`scimax.db.refresh'). [[cmd:scimax.db.refresh]]
...
- =scimax.db.reindex= is kept as an alias for backwards compatibility.
...
2. If files = 0, run `Scimax: Refresh Database (incremental)' (`scimax.db.refresh')
