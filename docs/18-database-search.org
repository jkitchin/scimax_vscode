...
Some paths are *always* ignored. This baseline cannot be turned off, because
indexing these directories produces duplicates of real notes rather than notes:
...
| Notebook checkouts | =**/.ipynb_checkpoints/**=                                     |
| System             | =**/.Trash/**=, =**/Library/Caches/**=                         |
| Editor state       | =**/Code/User/History/**=, =**/Code/User/globalStorage/**=,    |
|                    | =**/Code/User/workspaceStorage/**=, =**/Code/agent-host/**=,   |
|                    | the same paths for Code - Insiders, VSCodium and Cursor, and   |
|                    | =**/.vscode/extensions/**=                                     |

Editor state matters most. VS Code's *Local History* keeps a timestamped copy of
every edit under the original file extension, so a single =project.org= edited
200 times becomes 200 more =.org= files in the index — each with its own stale
copy of your TODOs, deadlines and scheduled items, all of which then appear in
the agenda.

=scimax.db.exclude= *adds* to this baseline; it does not replace it. That is
deliberate: a VS Code settings array overrides the contributed default outright,
so without a hard floor, adding one personal exclude pattern would silently
re-enable indexing of =node_modules= and editor history.

If entries that the current rules would reject are already in the database —
after an upgrade that adds a baseline pattern, or after you tighten
=scimax.db.exclude= — clear them with:

#+BEGIN_SRC sh
scimax db prune --dry-run   # show what would go, grouped by pattern
scimax db prune             # remove them
#+END_SRC

=scimax db sync= will not do this for you: it keeps a database entry whose file
still exists on disk, so that files added by other means are not lost.
...
2. *Embedding provider configured* - Ollama must be running locally
...
** Syncing for Semantic Search

After configuring an embedding provider, you *must re-sync* your files:

1. Run `Scimax: Sync Files' (`scimax.db.reindex').
   For a full embeddings rebuild, run `Scimax: Rebuild Database (from scratch)' (`scimax.db.rebuild').
...
CLOSED: [2026-01-27 Tue 18:11]

** Overview

Advanced search implements a state-of-the-art (SOTA) search pipeline inspired by modern search engines like [[https://github.com/tobi/qmd][qmd]]. It combines multiple techniques for maximum recall and precision:
...
*No LLM required* - Analyzes top results to find related terms.
...
*Requires Ollama* - Uses a language model to generate query variants.
...
*Requires Ollama* - Uses AI to score relevance after initial retrieval.
...
3. Pull models:
   #+BEGIN_SRC bash
   ollama pull qwen3:1.7b   # For query expansion
   ollama pull qwen3:0.6b   # For reranking
   #+END_SRC
4. Enable in settings:
   #+BEGIN_SRC json
   {
     "scimax.search.queryExpansion.method": "both",
     "scimax.search.reranking.enabled": true
   }
   #+END_SRC
...
Search headings by org-mode tags. This is the closest thing to Emacs
=org-tags-view=, and it doubles as a way to review the tags you already use.

*** Command
`Scimax: Search By Tag' (`scimax.db.searchByTag')

*** Features
- Lists every heading tag (=:tag:=) in the indexed files. =#hashtags= have
  their own command, =scimax.db.searchHashtags=.
- Shows how many headings carry each tag
- Sorted alphabetically, ignoring case, so spelling variants such as
  =:groupmeeting:= and =:groupmeetings:= sit next to each other
- Picking a tag lists headings that carry it directly or inherit it
- Respects the current search scope (all files or one directory)
- Tags displayed as `:tagname:'

The same tag list feeds =scimax.speed.setTags= (=C-c C-q=), which warns when
a new tag looks like one you already use. See
[[file:03-todo-items.org::*Tag Commands][Tag Commands]].
...
*** Sync Files (daily use)
Command: `Scimax: Sync Files' (`scimax.db.reindex'). [[cmd:scimax.db.reindex]]

- Scans all configured directories (=scimax.db.includeJournal=,
  =includeWorkspace=, =includeProjects=, plus =scimax.db.include=)
- Checks file modification times against the database
- Only re-indexes changed files
- This is the everyday command for picking up edits, especially when
  syncing across machines via Dropbox.
- =scimax.db.refresh= is the canonical implementation; =scimax.db.reindex= is the user-facing alias.

*** Full Rebuild
Command: `Scimax: Rebuild Database (from scratch)' (`scimax.db.rebuild'). [[cmd:scimax.db.rebuild]]

- Wipes the database and re-indexes everything
...
Add additional directories (or individual =.org=/=.md= files) with
=scimax.db.include=:
...
([[cmd:scimax.agenda.configure]]) appends to this list from a folder/file
picker and refreshes the index in one step.
...
*Additional* patterns or paths to exclude from indexing (globs and absolute
paths). These are added to the always-on baseline described under
[[*Ignore Patterns][Ignore Patterns]] — you do not need to repeat
=**/node_modules/**= and friends here.
...
| Command                       | Description                 |
|-------------------------------+-----------------------------|
| scimax.db.refresh             | Refresh database (incremental) |
| scimax.db.rebuild             | Rebuild database (from scratch) |
...
See [[file:17-projectile.org][Projectile]] for project management commands.
...
2. If files = 0, run `Scimax: Sync Files' (`scimax.db.reindex')
