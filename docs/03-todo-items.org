#+TITLE: TODO Items and Task Management
#+AUTHOR: Scimax VS Code Team
#+DATE: 2026-01-13
#+STARTUP: overview
#+OPTIONS: toc:2 num:t H:4
#+TODO: ⚠️ 👀 | ✅

*Keybinding Note:* This extension uses Emacs-style keybindings where Ctrl is used on all platforms (not Cmd on Mac), except where explicitly noted. Mac alternatives are shown as Cmd where applicable.

* ✅ TODO Items

Scimax VS Code provides a comprehensive task management system built into your
documents. TODO items are headlines with a special keyword that indicates their
status.

* ✅ Basic TODO Functionality

** ✅ Creating TODO Items

Add TODO after the headline stars to create a task:

#+BEGIN_SRC org
,* TODO Write documentation
,** TODO Complete first draft
,** TODO Review and edit
#+END_SRC

** ✅ Cycling TODO States

Press C-c C-t on a headline to cycle through TODO states:

#+BEGIN_EXAMPLE
(no keyword) → TODO → DONE → (no keyword)
#+END_EXAMPLE

Or use the command scimax.org.cycleTodo.

With speed commands enabled, press t at the beginning of a headline.

** ✅ TODO Keywords

The default TODO keywords are:

| Keyword | Meaning                    |
|---------+----------------------------|
| TODO    | Task that needs to be done |
| DONE    | Completed task             |

Additional states available:

| Keyword     | Meaning                              |
|-------------+--------------------------------------|
| NEXT       | Next action to take                  |
| WAITING    | Waiting for external input           |
| HOLD       | On hold / paused                     |
| CANCELLED  | Task cancelled (won't be done)       |

** ✅ Logging Completion
CLOSED: [2026-01-17 Sat 19:41]

When you mark a task as DONE, a CLOSED timestamp is automatically added:

#+BEGIN_SRC org
,* DONE Write introduction
CLOSED: [2026-01-13 Mon 15:30]
#+END_SRC

* ✅ Priorities

Priorities help you identify the most important tasks.

** ✅ Setting Priorities

Add a priority cookie after the TODO keyword:

#+BEGIN_SRC org
,* TODO [#A] Urgent and important task
,* TODO [#B] Important but not urgent
,* TODO [#C] Nice to have
#+END_SRC

** ✅ Priority Levels

| Priority | Meaning                         |
|----------+---------------------------------|
| [#A]     | High priority - do first        |
| [#B]     | Medium priority - do soon       |
| [#C]     | Low priority - do when possible |

** ✅ Priority Commands

| Key/Command                | Action                    |
|----------------------------+---------------------------|
| Speed key 1               | Set priority A            |
| Speed key 2               | Set priority B            |
| Speed key 3               | Set priority C            |
| Speed key 0               | Remove priority           |
| Speed key ,               | Increase priority (C→B→A) |

You can also set priority through the command palette:
scimax.org.setPriority

* ✅ Progress Indicators

Track progress through subtasks using statistics cookies and checkboxes.

** ✅ Statistics Cookies [/]

*** ✅ Fraction Format
CLOSED: [2026-01-17 Sat 19:42]

Shows completed/total count:


**** Project Tasks [2/4]
***** Task 1
***** Task 2
***** ✅ Task 3
***** ✅ [#C] Task 4


*** ✅ Percentage Format
CLOSED: [2026-01-17 Sat 19:47]

Shows completion percentage:

**** Project Progress [50%]
***** DONE Design
***** ✅ Implementation
CLOSED: [2026-01-17 Sat 19:47]
***** ✅ Testing
CLOSED: [2026-01-17 Sat 19:47]
***** TODO Deployment

Statistics cookies update automatically when child TODO states change.

*** ✅ Checkbox Lists

For simpler task lists within a section, use checkboxes:

* Shopping List [2/4]
- [X] Milk
- [X] Bread
- [ ] Eggs
- [x] Butter


*** ✅ Checkbox States

| Syntax | State                                   |
|--------+-----------------------------------------|
| [ ]    | Unchecked / not done                    |
| [X]    | Checked / done                          |
| [-]    | Partially complete (has mixed children) |

*** ✅ Toggling Checkboxes

Press C-c C-c with cursor on a checkbox line to toggle it.

Or use scimax.org.toggleCheckbox.

**** ✅ Nested Checkboxes

Checkboxes can be nested. Parent checkboxes show aggregate state:

#+BEGIN_SRC org
,* Packing List [1/2]
- [-] Clothes [2/3]
  - [X] Shirts
  - [X] Pants
  - [ ] Socks
- [X] Toiletries [3/3]
  - [X] Toothbrush
  - [X] Soap
  - [X] Towel
#+END_SRC

*** ✅ Inserting Checkboxes

| Command                   | Description               |
|---------------------------+---------------------------|
| scimax.org.insertCheckbox | Insert checkbox at cursor |

Type - [ ] followed by space to manually create a checkbox item.

* ✅ Extended TODO Keywords

Beyond TODO and DONE, use these states for workflow management:

** ✅ NEXT

Indicates the very next action to take:

#+BEGIN_SRC org
,* NEXT Call client about project requirements
#+END_SRC

Use NEXT for tasks you're actively working on or will work on immediately.

** ✅ WAITING

Task is blocked waiting for someone or something:

#+BEGIN_SRC org
,* WAITING Review from manager
:PROPERTIES:
:WAITING_ON: John Smith
:END:
#+END_SRC

** ✅ HOLD

Task is paused but not cancelled:

#+BEGIN_SRC org
,* HOLD Website redesign
Paused until Q2 budget approval.
#+END_SRC

** ✅ CANCELLED

Task won't be done (but kept for records):

#+BEGIN_SRC org
,* CANCELLED Old feature request
No longer relevant after product pivot.
#+END_SRC

** ✅ Workflow Example

A typical task progression:

#+BEGIN_EXAMPLE
TODO → NEXT → WAITING → NEXT → DONE
         ↓
       HOLD → NEXT → DONE
         ↓
     CANCELLED
#+END_EXAMPLE

* ✅ Custom TODO Keywords

You can define your own TODO workflow states for each file using the #+TODO:
keyword. This is useful for project-specific workflows like code review,
content publishing, or approval processes.

** ✅ Defining Custom States

Add #+TODO: at the top of your org file (before any headings):

#+BEGIN_SRC org
,#+TODO: DRAFT REVIEW | PUBLISHED REJECTED
#+END_SRC

The | separates active (incomplete) states from done (complete) states.
When you cycle TODO states, they will follow this sequence:

#+BEGIN_EXAMPLE
(none) → DRAFT → REVIEW → PUBLISHED → (none)
#+END_EXAMPLE

** ✅ Multiple TODO Lines

You can define multiple workflow sequences:

#+BEGIN_SRC org
,#+TODO: TODO IN-PROGRESS | DONE
,#+TODO: BUG INVESTIGATING FIXING | FIXED WONTFIX
#+END_SRC

All states from all #+TODO: lines are recognized.

** ✅ Without Pipe Separator

If you omit the |, the last state is treated as done:

#+BEGIN_SRC org
,#+TODO: IDEA DRAFT REVIEW PUBLISHED
#+END_SRC

This makes PUBLISHED the done state.

** ✅ Syntax Variants

These keywords are all equivalent:

| Keyword       | Usage                              |
|---------------+------------------------------------|
| #+TODO:      | Standard TODO keyword definition   |
| #+SEQ_TODO:  | Sequential workflow (same as TODO) |
| #+TYP_TODO:  | Type-based workflow                |

** ✅ Example Workflows

*** ✅ Code Review Workflow

#+BEGIN_SRC org
,#+TODO: DRAFT REVIEW APPROVED | MERGED REJECTED

,* DRAFT Implement new feature
,* REVIEW Fix login bug
,* APPROVED Update documentation
#+END_SRC

*** ✅ Content Publishing Workflow

#+BEGIN_SRC org
,#+TODO: IDEA OUTLINE WRITING EDITING | PUBLISHED ARCHIVED

,* WRITING Blog post about org-mode
,* EDITING Tutorial on source blocks
#+END_SRC

*** ✅ Project Management Workflow
#+BEGIN_SRC org
,#+TODO: BACKLOG SPRINT IN-PROGRESS TESTING | DONE CANCELLED

,* IN-PROGRESS User authentication
,* TESTING Payment integration
#+END_SRC

* ✅ Logging State Changes

Scimax VS Code can log when and how TODO states change.

** ✅ LOGBOOK Drawer
CLOSED: [2026-01-15 Thu 14:42]

State changes are recorded in the LOGBOOK drawer:

#+BEGIN_SRC org
,* DONE Complete documentation
CLOSED: [2026-01-13 Mon 16:00]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-01-13 Mon 16:00]
- State "TODO"       from "WAITING"    [2026-01-12 Sun 10:00]
- State "WAITING"    from "TODO"       [2026-01-10 Fri 14:30]
:END:
#+END_SRC

** ✅ Timestamps on State Changes
CLOSED: [2026-01-15 Thu 14:39]

When enabled, each state change adds:
- A timestamp showing when the change occurred
- The previous state

This creates a complete history of task progress.

* ✅ Progress Logging for Repeating Tasks
CLOSED: [2026-01-24 Sat 17:28]

When a repeating task cycles back to TODO (e.g., a task with =DEADLINE: <2026-01-30 Fri +1w>=),
you can optionally log the completion event with a timestamp and note.

** ✅ Enabling Progress Logging
CLOSED: [2026-01-24 Sat 17:28]

Progress logging is off by default. Enable it with:

*** ✅ Global Setting
CLOSED: [2026-01-24 Sat 17:41]

#+begin_src json
{
  "scimax.org.logRepeat": "time"  // or "note" for note prompts
}
#+end_src

*** ✅ File-Level Override
CLOSED: [2026-01-24 Sat 17:42]

Add a #+STARTUP keyword at the top of your org file:

#+begin_src org
,#+STARTUP: logrepeat       ;; Enable with timestamp
,#+STARTUP: lognoterepeat   ;; Enable with note prompt
,#+STARTUP: nologrepeat     ;; Disable
#+end_src

*** ✅ Heading-Level Override
CLOSED: [2026-01-24 Sat 17:28]

Add a =:LOGGING:= property to individual headings:

#+begin_src org
,* TODO Weekly review
:PROPERTIES:
:LOGGING: logrepeat
:END:
DEADLINE: <2026-01-30 Fri +1w>
#+end_src

** ✅ Priority Order
CLOSED: [2026-01-24 Sat 17:28]

Settings are resolved in this priority order (highest to lowest):
1. Heading =:LOGGING:= property
2. File =#+STARTUP:= keywords
3. Global =scimax.org.logRepeat= setting

** ✅ Log Output Example
CLOSED: [2026-01-24 Sat 17:28]

When you mark a repeating task as done with logging enabled:

#+begin_src org
,* TODO Weekly review
DEADLINE: <2026-02-06 Fri +1w>
:PROPERTIES:
:LAST_REPEAT: [2026-01-23 Fri 07:31]
:END:
- State "DONE"       from "TODO"       [2026-01-23 Fri 07:31]
#+end_src

The log shows:
- =LAST_REPEAT= property with when the task was last completed
- State change entry showing the transition from TODO to DONE

** ✅ Logging to Drawer
CLOSED: [2026-01-24 Sat 17:28]

By default, log entries appear in the body text. To log into a drawer:

#+begin_src json
{
  "scimax.org.logIntoDrawer": true       // Use :LOGBOOK: drawer
  // or
  "scimax.org.logIntoDrawer": "MYLOG"    // Use custom drawer name
}
#+end_src

Or use file-level =#+STARTUP: logdrawer= to enable.

With drawer logging, the output becomes:

#+begin_src org
,* TODO Weekly review
DEADLINE: <2026-02-06 Fri +1w>
:PROPERTIES:
:LAST_REPEAT: [2026-01-23 Fri 07:31]
:END:
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-01-23 Fri 07:31]
:END:
#+end_src

** ✅ Note Prompts
CLOSED: [2026-01-24 Sat 17:28]

When =logRepeat= is set to ="note"=, you'll be prompted to enter a note when
completing a repeating task. The note is included in the log entry:

#+begin_src org
- State "DONE"       from "TODO"       [2026-01-23 Fri 07:31] \\
  Completed early because of deadline change.
#+end_src

If you cancel or leave the note empty, the entry is logged without a note.

** ✅ Configuration Summary
CLOSED: [2026-01-24 Sat 17:28]

| Setting                  | Values                | Description                         |
|--------------------------+-----------------------+-------------------------------------|
| scimax.org.logRepeat     | "false"/"time"/"note" | Enable/disable repeat logging       |
| scimax.org.logIntoDrawer | boolean or string     | Where to log (body or drawer)       |
| #+STARTUP: logrepeat     | -                     | File-level: log with timestamp      |
| #+STARTUP: lognoterepeat | -                     | File-level: log with note prompt    |
| #+STARTUP: nologrepeat   | -                     | File-level: disable logging         |
| #+STARTUP: logdrawer     | -                     | File-level: use LOGBOOK drawer      |
| :LOGGING: logrepeat      | -                     | Heading-level: log with timestamp   |
| :LOGGING: lognoterepeat  | -                     | Heading-level: log with note prompt |
| :LOGGING: nologrepeat    | -                     | Heading-level: disable logging      |
| :LOGGING: nil            | -                     | Heading-level: disable logging      |

* ✅ Tags for TODO Items
CLOSED: [2026-01-15 Thu 14:39]

Tags provide additional categorization beyond the headline hierarchy.

** ✅ Adding Tags
CLOSED: [2026-01-15 Thu 14:39]

Tags appear at the end of the headline, enclosed in colons:

#+BEGIN_SRC org
,* TODO Review quarterly report                               :work:urgent:
,* TODO Buy groceries                                         :personal:errands:
#+END_SRC

** 👀 Tag Commands

| Key/Command                   | Action                                 |
|-------------------------------+----------------------------------------|
| Speed key =:= (S-;)           | Set tags on the heading                |
| C-c C-q                       | Set tags on the heading (org files)    |
| [[cmd:scimax.speed.setTags]]  | Set tags via command                   |
| [[cmd:scimax.db.searchByTag]] | List all tags in use, jump to headings |

Setting tags opens a multi-select list of every tag already in use: the tags
in the current file and its =#+TAGS:= / =#+FILETAGS:= lines, plus every
heading tag in the database, each with how many headings carry it. Check or
uncheck tags and press =Enter=. The heading's current tags start out checked.

To add a tag nobody has used yet, type it. It shows up at the top as a
"new tag". If it looks like an existing tag (a plural, a typo, or a
different capitalization), the entry warns you, for example:

#+BEGIN_EXAMPLE
groupmeetings   new tag
  ⚠ similar to existing: :groupmeeting:
#+END_EXAMPLE

That makes it easy to reuse =:groupmeeting:= instead of starting a second
spelling of it.

** ✅ Inherited Tags
CLOSED: [2026-01-15 Thu 14:39]

Child headlines inherit their parent's tags:

#+BEGIN_SRC org
,* Work Projects                                              :work:
,** TODO Project A                                            :client1:
This task has both :work: and :client1: tags.
,** TODO Project B                                            :internal:
This task has both :work: and :internal: tags.
#+END_SRC

** ✅ Common Tag Conventions
CLOSED: [2026-01-15 Thu 14:42]

| Tag       | Usage                   |
|-----------+-------------------------|
| @office   | Context: at office      |
| @home     | Context: at home        |
| @computer | Context: needs computer |
| @phone    | Context: phone call     |
| urgent    | Priority indicator      |
| project   | Part of larger project  |

** 👀 Filtering by Tags

Use agenda views or search to filter tasks by tag:

- scimax.db.searchByTag - List every heading tag in the database with its
  count (like Emacs =org-tags-view=), then pick one to see the headings that
  carry it, including headings that inherit it from a parent
- scimax.agenda.filterByTag - Filter agenda by tag

* ✅ Scheduling and Deadlines
CLOSED: [2026-01-15 Thu 14:40]

Assign dates to tasks for planning.

** ✅ SCHEDULED
CLOSED: [2026-01-15 Thu 14:40]

When you plan to start or work on a task:

#+BEGIN_SRC org
,* TODO Write report
SCHEDULED: <2026-01-15 Wed>
#+END_SRC

** ✅ DEADLINE
CLOSED: [2026-01-15 Thu 14:40]

When a task must be completed by:

#+BEGIN_SRC org
,* TODO Submit proposal
DEADLINE: <2026-01-20 Mon>
#+END_SRC

** ✅ Both Together
CLOSED: [2026-01-15 Thu 14:40]

#+BEGIN_SRC org
,* TODO Complete project
SCHEDULED: <2026-01-15 Wed> DEADLINE: <2026-01-25 Sat>
#+END_SRC

** ✅ Adding Dates
CLOSED: [2026-01-15 Thu 14:40]

| Speed Key | Action                  |
|-----------+-------------------------|
| s         | Add/edit SCHEDULED date |
| d         | Add/edit DEADLINE date  |
| .         | Insert timestamp        |

Or use commands:
- scimax.org.schedule
- scimax.org.deadline

See [[file:06-timestamps.org][Timestamps]] for more on date formats.

* ✅ Time Tracking
CLOSED: [2026-01-15 Thu 14:40]

Track time spent on tasks using clock entries.

** ✅ Clocking In/Out
CLOSED: [2026-01-15 Thu 14:40]
:LOGBOOK:
CLOCK: [2026-01-15 Thu 14:40]--[2026-01-15 Thu 14:43] => 0:03
CLOCK: [2026-01-15 Thu 14:40]--[2026-01-15 Thu 14:40] => 0:00
:END:

| Speed Key | Action                  |
|-----------+-------------------------|
| I   | Clock in (start timing) |
| O   | Clock out (stop timing) |

** ✅ Clock Entries
CLOSED: [2026-01-15 Thu 14:41]

Clock entries are stored in the LOGBOOK drawer:

#+BEGIN_SRC org
,* TODO Working on documentation
:LOGBOOK:
CLOCK: [2026-01-13 Mon 09:00]--[2026-01-13 Mon 10:30] =>  1:30
CLOCK: [2026-01-12 Sun 14:00]--[2026-01-12 Sun 16:00] =>  2:00
:END:
#+END_SRC

** ✅ Total Time
CLOSED: [2026-01-15 Thu 14:41]

The total clocked time appears in the properties or can be calculated for
reports.

* ✅ Effort Estimates
CLOSED: [2026-01-15 Thu 14:41]

Estimate how long tasks will take.

** ✅ Setting Effort
CLOSED: [2026-01-15 Thu 14:41]
:PROPERTIES:
:Effort: 0:30
:END:

Add an EFFORT property:

#+BEGIN_SRC org
,* TODO Review document
:PROPERTIES:
:EFFORT: 1:30
:END:
#+END_SRC

| Speed Key | Action              |
|-----------+---------------------|
| e         | Set effort estimate |

** ✅ Effort Format
CLOSED: [2026-01-15 Thu 14:41]

Use H:MM format:
- 0:30 - 30 minutes
- 1:00 - 1 hour
- 2:30 - 2 hours 30 minutes

* ✅ Agenda Integration
CLOSED: [2026-01-15 Thu 14:41]

TODO items appear in agenda views for planning and review.

** ✅ Viewing All TODOs
CLOSED: [2026-01-15 Thu 14:41]

Use scimax.agenda.todoList or scimax.db.showTodos to see all tasks.

** ✅ Deadline View
CLOSED: [2026-01-15 Thu 14:41]

Use scimax.agenda.deadlines to see upcoming and overdue deadlines.

** ✅ Filtering Options
CLOSED: [2026-01-15 Thu 14:41]

- By priority (A, B, C)
- By tag
- By date range
- By TODO state

See [[file:13-agenda.org][Agenda]] for complete agenda documentation.

* ✅ Best Practices
CLOSED: [2026-01-15 Thu 14:41]

** ✅ Task Granularity
CLOSED: [2026-01-15 Thu 14:41]

Make tasks actionable:

#+BEGIN_SRC org
,* TODO "Write documentation"           ; Too vague
,* TODO Write introduction section      ; Better - specific and actionable
#+END_SRC

** ✅ Use NEXT for Clarity
CLOSED: [2026-01-15 Thu 14:41]

Always have at least one NEXT task to know what to work on:

#+BEGIN_SRC org
,* Project: New Feature
,** DONE Design architecture
,** NEXT Implement core module           ; ← Clear next step
,** TODO Write tests
,** TODO Deploy to staging
#+END_SRC

** ✅ Regular Review
CLOSED: [2026-01-15 Thu 14:41]

Periodically review tasks to:
- Move stale TODO items to HOLD or CANCELLED
- Update WAITING items
- Identify next actions

** ✅ Tags vs Hierarchy
CLOSED: [2026-01-15 Thu 14:41]

Use hierarchy for project structure, tags for cross-cutting concerns:

#+BEGIN_SRC org
,* Work                                                       :work:
,** Project A
,*** TODO Task requiring phone                                :@phone:
,*** TODO Task requiring computer                             :@computer:
,** Project B
,*** TODO Another phone task                                  :@phone:
#+END_SRC

Now you can filter by @phone to see all phone tasks across projects.

* ✅ Quick Reference
CLOSED: [2026-01-15 Thu 14:41]

** ✅ Keybindings
CLOSED: [2026-01-15 Thu 14:41]

| Key           | Action           |
|---------------+------------------|
| C-c C-t | Cycle TODO state |
| C-c C-c | Toggle checkbox  |

** ✅ Speed Commands (at heading start)
CLOSED: [2026-01-15 Thu 14:42]

| Key     | Action             |
|---------+--------------------|
| t       | Cycle TODO state   |
| 1/2/3   | Set priority A/B/C |
| 0       | Clear priority     |
| ,       | Increase priority  |
| s       | Schedule           |
| d       | Deadline           |
| e       | Set effort         |
| ; | Set tags           |
| I | Clock in           |
| O | Clock out          |

** ✅ Commands
CLOSED: [2026-01-15 Thu 14:42]

| Command                   | Description             |
|---------------------------+-------------------------|
| scimax.org.cycleTodo      | Cycle TODO state        |
| scimax.org.toggleCheckbox | Toggle checkbox         |
| scimax.org.insertCheckbox | Insert checkbox         |
| scimax.org.schedule       | Add SCHEDULED timestamp |
| scimax.org.deadline       | Add DEADLINE timestamp  |
| scimax.db.showTodos       | Show all TODO items     |
| scimax.agenda.todoList    | Open TODO agenda view   |

* ✅ Related Topics
CLOSED: [2026-01-15 Thu 14:42]

- [[file:03.5-task-dependencies.org][Task Dependencies]] - Blocking, triggering, and ORDERED subtasks
- [[file:06-timestamps.org][Timestamps]] - Date formats for scheduling and deadlines
- [[file:13-agenda.org][Agenda]] - Viewing and filtering tasks
- [[file:02-document-structure.org][Document Structure]] - Headlines and organization

* Navigation

- Previous: [[file:02-document-structure.org][Document Structure]]
- Index: [[file:00-index.org][Documentation Index]]
- Next: [[file:04-tables.org][Tables]]
