...
description: |
  ...
  back-links, source blocks, tables, citations, task dependencies, and project
  management: task tables, Gantt charts, people/assignees). Use when the user
  works with org-mode notes, asks about org syntax, or runs scimax CLI tasks.
...
3. **Read org-mode reference** — When helping with org-mode syntax, citations, source blocks, document structure, links, granular addressing (anchors, radio targets, `#+NAME`, back-links, orphan diagnostics), task dependencies (`:DEPENDS:`, `:ORDERED:`, blocking/triggering), or project management (`project-table`/`gantt` dynamic blocks, `:person:`/`:ASSIGNEE:`), use the Read tool to load `~/.claude/skills/scimax/orgmode-reference.md`.
