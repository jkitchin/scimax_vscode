...
import { NON_ASCII_MAP, NonAsciiReplacement } from '../utils/nonAsciiMap';
import {
    findPlanningLine,
    buildPlanningLine,
    removeClosed,
...
// NON_ASCII_MAP and the NonAsciiReplacement type live in ../utils/nonAsciiMap
// (a dependency-free module shared with the LaTeX/HTML export escaping layer).

// Session storage for user-defined replacements
const userDefinedReplacements: Record<string, NonAsciiReplacement> = {};
...
export async function cycleTodoState(): Promise<void> {
    ...
    if (await guardCompletion(document, position.line, doneStates, wasDone, isNowDone)) {
        return;
    }
