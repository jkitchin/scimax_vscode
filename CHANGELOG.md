...
- **Deflaked the test suite under parallel runs** - the parser benchmarks now measure CPU time instead of wall-clock time (parallel test files oversubscribe the CPU and inflated wall time past the 1.25x baseline tolerance), and the Jupyter suite gets a 15s per-test timeout because most of its tests shell out to the multi-second `jupyter` CLI.
